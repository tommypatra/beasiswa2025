...
use App\Http\Requests\SkPenerimaRequest;
...
class SkPenerimaController extends Controller
{
     ...
     */
    public function store(SkPenerimaRequest $request)
    {
        try {
            DB::beginTransaction();
            $data = SkPenerima::create($request->validated());
            DB::commit();
     ...
     */
    public function update(SkPenerimaRequest $request, string $id)
    {
        try {
            DB::beginTransaction();
            $data = SkPenerima::where('id', $id)->firstOrFail();
