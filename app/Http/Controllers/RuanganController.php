...
class RuanganController extends Controller
{
    ...
    {
        $dataQuery = Ruangan::orderBy('nama', 'asc');
    ...
    public function store(RuanganRequest $request)
    {
        try {
            DB::beginTransaction();
            $data = Ruangan::create($request->validated());
            DB::commit();
    ...
    {
        try {
            $dataQuery = Ruangan::where('id', $id)->firstOrFail();
            return response()->json([
                'status' => true,
                'message' => 'Data ditemukan',
                'data' => new RuanganResource($dataQuery),
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'status' => false,
    ...
    public function update(RuanganRequest $request, string $id)
    {
        try {
            DB::beginTransaction();
            $data = Ruangan::where('id', $id)->firstOrFail();
