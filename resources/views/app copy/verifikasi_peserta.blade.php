...
@section('scriptHead')
<title>Verifikasi Peserta Seleksi Beasiswa</title>
<style>
...
<div class="card">
    <div class="card-body">
        <div class="d-sm-flex d-block align-items-center justify-content-between mb-3">
            <h5 class="card-title fw-semibold">Verifikasi Peserta Seleksi Beasiswa</h5>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
            ...
            if (data.length > 0) {
                $.each(data, function(index, dt) {
                    ...
                    const row = `<tr>
                                    ...
                                    <td>
                                        ${dt.verifikasi_berkas_mulai} sd 
                                        ${dt.verifikasi_berkas_selesai}
                                    </td>
                                    <td>
                                        <span class="badge rounded-pill fs-2 fw-medium bg-secondary-subtle text-secondary">Jumlah Pendaftar : ${dt.pendaftar_count}</span>
                                        <span class="badge rounded-pill fs-2 fw-medium bg-success-subtle text-success">Finalisasi : ${dt.pendaftar_count}</span>
                                        <span class="badge rounded-pill fs-2 fw-medium bg-danger-subtle text-danger">Batal : ${dt.pendaftar_batal}</span>
                                    </td>
                                    <td>
                                        ...
                                        <span class="badge rounded-pill fs-2 fw-medium bg-success-subtle text-success">MS : ${dt.verifikasi_ms}</span>
                                        <span class="badge rounded-pill fs-2 fw-medium bg-danger-subtle text-danger">TMS : ${dt.verifikasi_tms}</span>
                                    </td>
                                    <td class="text-center">
                                        <div class="d-flex flex-column align-items-center gap-2">
                                            <a href="${base_url}/dashboard-beasiswa/${dt.id}" class="btn btn-secondary">Selengkapnya</a>
                                        </div>
                                    </td>
                                </tr>`;
                    dataList.append(row);
        ...
        async function dataLoad() {
            ...
            let url = `${base_url}/api/pengelola/peserta-verifikasi?search=${search}`;
